...
ds= xr.open_dataset(data_path_era5).sel(time=(slice(tstart, tfinal)), expver=1)
...
def pr_at_index(ds, x_ind, y_ind):
    ...
    ds2=ds.isel(longitude=x_ind,
        latitude= y_ind)
    da = ds2.tp * 1000
    da.attrs={'units': 'mm', 'long_name': 'Total precipitation', 'data_source':'Era5' }
...
def pev_at_index(ds, x_ind, y_ind):
    ...
    da= ds2.pev * -1000
    da.attrs={'units': 'mm', 'long_name': 'Potential evaporation', 'data_source':'Era5'}
    return da
...
ds_spi_era5= xr.merge([da_spi1,
                           da_spi3,
                           da_spi6])

ds_spi_era5.attrs['source_variable_tp'] = 'tp: ERA5 xpver1 Monthly Mean Total Precipitation in mm (org tp *1000)'
ds_spi_era5.attrs['SPI_function'] = f'R package SPEI, distibution={distribution}, reference period {spei_ref[0]}-{spei_ref[1]}'


out_spi = '/home/evelien/Agri-SiSa/data/output/ERA5_SPI136_1990_2022_NL.nc' 
ds_spi_era5.to_netcdf(out_spi)
...
ds_spei_era5= xr.merge([da_spei1,
                           da_spei3,
                           da_spei6])
...
ds_spei_era5.attrs['source_variable_tp'] = 'tp: ERA5 Expver1 Monthly Mean Total Precipitation in mm (org tp *1000)'

out_spei = '/home/evelien/Agri-SiSa/data/output/ERA5_SPEI136_1990_2022_NL.nc' 
ds_spei_era5.to_netcdf(out_spei)
#%%

era5land = xr.open_dataset('/home/evelien/Agri-SiSa/data/output/ERA5land_SPEI136_1990_2022_NL.nc')
